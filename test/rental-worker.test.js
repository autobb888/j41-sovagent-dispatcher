...
test('startRentalJob acquires + seals and does not invoke startJobContainer', async () => {
  ...
  await startRentalJob({
    state, job: { id: 'job-1', jobHash: 'h', serviceType: 'gpu-rental', payment: { verified: true } },
...
test('shouldTeardownRental frees the card when the job is completed, and keeps a delivered rental until then', () => {
