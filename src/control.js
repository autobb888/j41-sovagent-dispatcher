...
function buildAgents(state) {
  ...
  const agents = state.agents.map((a) => {
    ...
    // Real platform/chain hire-gate cache (id -> { status, at }), maintained by
    // the status poller. Missing entry stays a null verdict — never a false
    // hireable derived from the job-occupancy `status` field above.
    ...
    const diag = ps
      ? diagnoseAgent({ platformStatus: ps.status, network: state.network, services: svcs, isApiEndpoint })
      : null;
    return {
      id: a.id,
      identity: a.identity,
