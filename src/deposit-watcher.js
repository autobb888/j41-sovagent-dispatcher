...
async function _reportVerifiedDeposit(agentId, client, report, payAddress, network) {
  ...
  try {
    ...
    if (verification.senderVerified === undefined) {
      ...
      // identity funded the transaction.
      //
      // There used to be a J41_DEPOSIT_ALLOW_AUTH_ONLY=1 escape hatch here for
      // platforms that could not return `senderVerified`. Removed 2026-08-14:
      // the backend has advertised `tx.sender-verification` for months, so the
      // legacy case it existed for no longer has a user — and an env var that
      // switches OFF a verification check is the exact pattern this project
      // decided not to ship. Opt in to stricter, never opt out of safe.
      return {
        credited: false,
        code: 'SENDER_VERIFICATION_REQUIRED',
        message: 'Platform did not return sender verification for this deposit; refusing to credit on ' +
          'signature-only auth. Signing a report proves control of the buyer identity, not that it funded the transaction.',
      };
    }
